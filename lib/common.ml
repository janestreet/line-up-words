...
let combine_triples (triples : MB.t list list) =
  ...
  | [] -> []
  | head :: tail ->
    List.fold tail ~init:head ~f:(fun acc l -> Interval.list_intersect acc l)
;;
...
let combine triples =
  ...
  let intervals =
    List.filter intervals ~f:(fun interval -> not (Interval.is_empty interval))
  in
  List.map intervals ~f:(fun interval ->
    let ids =
      List.map triples ~f:(fun triples -> find_idx triples (I.lbound_exn interval))
    in
...
let split s =
  ...
  let list =
    List.group list ~break:(fun res1 res2 ->
      match res1, res2 with
      | Pcre.Text _, Pcre.Delim s -> String.strip s = ""
      | _ -> true)
  in
  List.map list ~f:(fun split_results ->
    let strings =
      List.map split_results ~f:(function
...
let%expect_test "example" =
  print_s
    [%sexp
      (foldi_right [ 4; 5; 6 ] ~f:(fun i a b -> (i, a) :: b) ~init:[] : (int * int) list)];
  [%expect {| ((0 4) (1 5) (2 6)) |}]
;;
...
let split_respecting_quotations s =
  ...
  in
  foldi_right alternating_literals_and_quotations ~init:[] ~f:(fun i s words ->
    if i % 2 = 0
    then (* "literal" *)
      split s @ words
    else (
  ...
  fun line ->
    ...
    then false
      ...
      let non_comments_rewritten =
        let line = Re2.rewrite_exn open_paren_noncomment_rex line ~template:"l\\1" in
      ...
      in
      try
        ignore (Sexp.of_string comments_rewritten : Sexp.t);
        true
      with
      | _ -> false)
;;
...
let line_up_words lines =
  let non_comment_lines, comment_lines =
    ...
    let non_comment_lines =
      Array.map2_exn lines_to_ignore lines ~f:(fun line_is_comment line ->
        if line_is_comment then None else Some line)
      |> Array.filter_opt
      |> Array.to_list
    in
    let comment_lines =
      Array.map2_exn lines_to_ignore lines ~f:(fun line_is_comment line ->
        if line_is_comment then Some line else None)
      |> Array.filter_mapi ~f:(fun i opt -> Option.map opt ~f:(fun x -> i, x))
      |> Array.to_list
    in
  ...
  | mine :: others ->
    ...
    let nonmatch_ids =
      ...
      let normalized_ids =
        let last =
          ( List.init num_lines ~f:(fun i -> List.length (List.nth_exn word_split_lines i))
          , 0 )
        in
        normalized_ids @ [ last ]
      in
...
module%test _ = struct
  ...
  let%expect_test "strings get treated like symbols when binding to commas" =
    print_s
      [%sexp
        (split_respecting_quotations {| [ "analyze", analyze_command |} : string list)];
    [%expect {| ("" [ "" "\"analyze\"," "" analyze_command "") |}];
    print_s
      [%sexp (split_respecting_quotations {| [ analyze, analyze_command |} : string list)];
    [%expect {| ("" [ "" analyze, "" analyze_command "") |}]
  ;;
